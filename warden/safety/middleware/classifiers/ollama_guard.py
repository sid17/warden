"""Ollama input classifier — Qwen3Guard via local Ollama API."""
...
class OllamaGuardInputClassifier:
    """Input classifier using Ollama-hosted LLM for prompt injection detection."""

    name = "ollama-gemma3-input"
...
def _parse_response(raw: str) -> tuple[str, float]:
    ...
    try:
        ...
        # If model didn't return valid JSON, try keyword detection
        lower = text.lower()
        if "unsafe" in lower:
            return "unsafe", 0.5
        if "safe" in lower:
